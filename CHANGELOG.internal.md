...
- `AppServerCodexBackend`: drives Codex via the experimental `codex app-server` JSON-RPC protocol (newline-delimited JSON-RPC 2.0 over stdio) with `initialize`→`thread/start`/`thread/resume`→`turn/start` lifecycle, app-server v2 item translation (camelCase→normalized), and `turn/steer` for mid-turn input injection. Gated behind `EdgeWorkerConfig.codexUseAppServer` / `CODEX_USE_APP_SERVER=1`; default off because the underlying app-server protocol is marked experimental by Codex. Includes replay tests built from real `codex app-server` transcripts and an injectable client (`IAppServerClient`) for hermetic backend tests. Experiment scripts and a full requirements/design scope live in `packages/codex-runner/experiments/`.
