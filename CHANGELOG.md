...
### Changed
- **Updated `@anthropic-ai/claude-agent-sdk` from 0.2.123 to 0.3.143 and `@anthropic-ai/sdk` from ^0.91.0 to ^0.96.0** — Brings in Claude Code v2.1.124–2.1.143 and a major SDK release (0.3.x). Key highlights: Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) now replace the deprecated `TodoWrite`; MCP servers now connect in the background by default so sessions start immediately; the v2 session API has been removed (use `query()` instead); `@anthropic-ai/sdk` is now a peer dependency. See the [claude-agent-sdk changelog](https://github.com/anthropics/claude-agent-sdk-typescript/blob/main/CHANGELOG.md) for full details. ([CYPACK-1211](https://linear.app/ceedar/issue/CYPACK-1211), [#1222](https://github.com/cyrusagents/cyrus/pull/1222))
